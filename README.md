...
2. 浏览器会自动打开本地工具页面。
3. 粘贴或上传坐标 CSV，字段为 `site_id,name,latitude,longitude,country`。
4. 设置年份、时间标准和超温阈值，默认 `2025 / LST / 40°C`。
5. 点击“开始拉取”，完成后下载一个 Excel 工作簿。

网页会直接请求 NASA POWER API，并使用浏览器 IndexedDB 缓存。缓存只保存在当前浏览器；换电脑、换浏览器或清空站点数据后不会自动迁移。

在线版本支持 GoatCounter 匿名聚合统计，用于了解访问量和实际使用次数。统计只记录页面访问、开始拉取、完成拉取、运行失败和下载 Excel 这类事件，不记录城市名、经纬度、CSV 内容、缓存内容或导出结果。配置方式：

1. 在 https://www.goatcounter.com/ 创建站点，站点地址填 `https://EOShoow.github.io/temperature-tool/`。
2. 复制 GoatCounter 的 `code`。
...
网页默认开启“自动双源校验”：NASA 主数据拉取完成后，浏览器会自动请求 Open-Meteo ERA5-Land `temperature_2m` 的固定三周窗口（每年 `01-01~01-07`、`05-01~05-07`、`10-01~10-07`），并把城市级 `双源一致 / 双源基本一致，高温窗口需标注 / 需第三/第四源投票` 状态合入摘要和 Excel。

复制到本地运行时，建议保留整个项目文件夹。至少需要 `web/`、`tools/`、`启动温度工具-Windows.bat`、`启动温度工具-Mac.command` 保持相对位置不变。
...
直接双击 `web/index.html` 可作为兜底，但 `file://` 下 IndexedDB 缓存、联网请求和 CDN 脚本在不同浏览器上不如本地服务稳定。
...
“搜国家（代表城市）”使用工具内置候选表，不再实时扫描整国城市；因此未维护国家会给出提示，而不会触发 `Load failed`。
...
- 年份范围：网页默认 `2025`；需要多年统计时可手动改为 `2016-2025` 等完整自然年范围
